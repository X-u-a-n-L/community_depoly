...
function collapseComment(e) {
    var id = e.getAttribute("data-id");//get comment id(数字)
    var comments = $("#comment-" + id);  //对id为$("#comment-" + id)的前端语句进行动态操作
    var comment_active = $("#secondary_comment_active" + id);
    //展开或折叠二级评论
    comments.toggleClass("in");
    comment_active.toggleClass("active");//set secondary comment to active(blue)

    if (comments.hasClass("in")) {
        var subCommentContainer = $("#comment-" + id);
        if (subCommentContainer.children().length == 1) {   //为了不让二级内容重复的append
